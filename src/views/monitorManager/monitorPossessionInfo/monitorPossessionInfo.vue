    ...
    <template>
      <div class="card dark-main-background">
        ...
        >
          <div class="queryleft">
            <p class="title-bar-description" style>
              <span>{{this.$route.query.hostName}}\</span>
            </p>
          </div>
        ...
        <div class="tempList card-body">
          ...
          <div>
            <div class="card m-r">
              <div class="card-body p-a-0"></div>
              <div class="card-footer no-border text-center"></div>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    showInfoTimeout (str) {
      this.axios.post('/host/findById/' + this.$route.query.hostId).then((resp) => {
        if (resp.status === 200) {
          var json = resp.data
          if (json.code === 1) {

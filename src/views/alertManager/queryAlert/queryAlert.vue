...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      this.axios.post(api.alertManager.queryAlert.queryAlert.updateTemplates, param).then((resp) => {
        if (resp.status === 200) {
          var json = resp.data
          if (json.code === 1) {
            this.$message({
    ...
    getTemplates () {
      this.axios.post(api.alertManager.queryAlert.queryAlert.getZabbixTemplates).then((resp) => {
        if (resp.status === 200) {
          var json = resp.data
          if (json.code === 1) {
    ...
    checkItem (value) {
      ...
      this.axios.post(api.alertManager.queryAlert.queryAlert.checkItems, param).then((resp) => {
        if (resp.status === 200) {
          var json = resp.data
          if (json.code === 1) {
